...
# -*- coding: utf-8 -*-
"""
test_dashboard_toggle.py – Dummy Dashboard 🌿 v3
Schalter für 3↔6 Tiles, nutzt grünes Raster-Hintergrundbild.
...
class Tile(FloatLayout):
    ...
    def _build_ui(self):
        # Hintergrundbild – fill mode, volle Abdeckung
        ...
        # Leicht transparente Headerleiste
        with self.canvas.before:
            Color(0, 0, 0, 0.25)
            self._hdr = Rectangle(size=(self.width, 28), pos=(self.x, self.top - 28))
    ...
    def _update_hdr(self, *args):
        self._hdr.size = (self.width, 28)
        self._hdr.pos = (self.x, self.top - 28)
...
class DummyDashboard(BoxLayout):
    ...
    def _build_grid(self):
        ...
        cols = 2 if self.tile_count == 6 else 1
        self._grid = GridLayout(cols=cols, padding=10, spacing=10)
